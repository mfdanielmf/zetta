...
from app.repositories.user_repo import insert_user_db, get_user_by_email, get_user_by_id, get_user_by_name
...
def crear_usuario(usuario: UserCreate, db: Session) -> User:
    if get_user_by_name(nombre=usuario.nombre, db=db) is not None:
        raise HTTPException(
            status_code=400, detail=f"Ya se ha encontrado un usuario con nombre {usuario.nombre}")

    if get_user_by_email(correo=usuario.correo, db=db) is not None:
        raise HTTPException(
            status_code=400, detail=f"Ya se ha encontrado un usuario con el correo {usuario.correo}")
