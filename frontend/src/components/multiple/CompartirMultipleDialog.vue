...
const emit = defineEmits(['compartirSeleccionado'])
...
const onSubmit = handleSubmit((data) => {
  emit('compartirSeleccionado', data.correo)
})
</script>
...
<template>
  <Dialog>
    <DialogContent class="sm:max-w-106.25">
      <DialogHeader>
        <DialogTitle>Compartir elementos</DialogTitle>
        <DialogDescription>
          Comparte los elementos seleccionados con otro usuario
        </DialogDescription>
      </DialogHeader>
      ...
      <DialogFooter>
        <Button class="hover:cursor-pointer" @click="onSubmit" :disabled="props.pending">
          ...
          <Share2 v-else />
          {{ props.pending ? 'Compartiendo...' : 'Compartir' }}
        </Button>
      </DialogFooter>
    </DialogContent>
  </Dialog>
